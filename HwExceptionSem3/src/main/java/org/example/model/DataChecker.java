...
public class DataChecker {
    ...
    public void CheckData(String[] splitedString) throws ParsingDataStringException {
        ...
        for (String string : splitedString) {
            if (Character.isLetter(string.charAt(0))) {
                if (string.length() == 1) {
                    ...
                    } else if (this.patronomicName == null) {
                        try {
                            this.patronomicName = checkFIO(string);
                        } catch (BadFIOException e) {
                            fullErrorsMessages.append(e.getMessage());
                        }
                    } else {
        ...
        if (fullErrorsMessages.length() !=0) {  //??? было так !fullErrorsMessages.isEmpty()
    ...
    private String checkFIO(String inputString) throws BadFIOException {
        if (inputString.toLowerCase().matches("^[a-za-яё]*$")) {
            return inputString;
        } else {
    ...
    public String toString() {
        ...
        sb.append("<").append(lastName).append(">")
                ...
                .append("<").append(patronomicName).append(">")
